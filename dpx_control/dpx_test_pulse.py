...
class DPXtestPulse(object):
    ...
    def testPulseClose(self, slot):
        # Reset ConfBits
        self.DPX_write_configuration_command(slot, self.conf_bits[slot - 1])

        # Reset peripheryDACs
        self.DPX_write_periphery_DAC_command(
            slot, self.peripherys + self.THLs[slot - 1])
    ...
    def maskBitsColumn(self, slot, column):
        ...
        conf_bits = np.zeros((16, 16))
        conf_bits.fill(getattr(ds._ConfBits, 'MaskBit'))
        conf_bits[column] = [getattr(ds._ConfBits, 'TestBit_Analog')] * 16

        # conf_bits = np.asarray( [int(num, 16) for num in textwrap.wrap(self.conf_bits[slot-1], 2)] )
        # conf_bits[conf_bits != getattr(ds._ConfBits, 'MaskBit')] = getattr(ds._ConfBits, 'TestBit_Analog')

        self.DPX_write_configuration_command(slot, ''.join(
            ['%02x' % conf for conf in conf_bits.flatten()]))

    def maskBitsPixel(self, slot, pixel=(0, 0)):
        conf_bits = np.zeros((16, 16))
        conf_bits.fill(getattr(ds._ConfBits, 'MaskBit'))
        conf_bits[pixel[0]][pixel[1]] = getattr(ds._ConfBits, 'TestBit_Analog')
        self.DPX_write_configuration_command(slot, ''.join(
            ['%02x' % conf for conf in conf_bits.flatten()]))
        ...
        for column in columnRange:
            ...
            for val in valueRange:
                ...
                for i in range(NToT):
                    ...
                    data = self.DPX_read_ToT_data_dosimode_command(slot)[column]
                ...
                # THLRange = self.THL_calib[np.logical_and(self.THL_calib > THLlow, self.THL_calib < THLhigh)]
                ...
                THLRange = THLRange[np.logical_and(
                    THLRange >= THLstart, THLRange <= THLstop_)]

                # Do a fast measurement to estimate the position of the edge
                # for each pixel
                THLFastList = []
                dataFastList = []
                THLFastStep = 5
                ...
                for cnt, THL in enumerate(
                        THLRange[::THLFastStep]):  # :THLFastStep]):
                    # Set new THL
                    self.DPX_write_periphery_DAC_command(
                        slot, DACval[:-4] + '%04x' % THL)
                ...
                dataFastDer = [xFastDer[np.argmax(np.diff(data) / float(THLFastStep))]
                               for data in np.asarray(dataFastList).T if np.count_nonzero(data)]
                ...
                regSel = np.logical_and(
                    THLRange >= min(dataFastDer) - THLFastStep,
                    THLRange <= max(dataFastDer) + THLFastStep)
                ...
                dataDer = [np.diff(data) / float(THLstep)
                ...
                for data in np.asarray(dataList).T:
                    ...
                    THLListCorr = [
                        self.getVoltFromTHLFit(
                            elm, slot) for elm in THLList]
                    THLListFit = np.linspace(
                        min(THLListCorr), max(THLListCorr), 1000)
                    ...
                    try:
                        popt, pcov = scipy.optimize.curve_fit(
                            lambda x, b, c: self.erfFit(
                                x, NPulses, b, c), THLListCorr, data, p0=p0)
                    ...
                    if plot:
                        ...
                        plt.plot(
                            THLListFit, self.erfFit(
                                THLListFit, NPulses, b, c), ls='-')
                        plt.plot(
                            THLListFit, self.normalErf(
                                THLListFit, NPulses, b, c), ls='-')
                        plt.plot(THLListCorr, data, marker='x', ls='')
                        plt.show()

                peakList, peakErrList = np.asarray(
                    peakList), np.asarray(peakErrList)
                ...
                print('Start, Stop')
                THLstart, THLstop = np.nanmin(
                    THLList) - 400, np.nanmax(THLList) + 400
            ...
            resDict[column] = {
                'THL': THLListTotal,
                'THLErr': THLErrListTotal,
                'ToT': ToTListTotal,
                'ToTErr': ToTErrListTotal,
                'volt': valueRange}
        ...
        if plot:
            ...
            for i in range(len(THLListTotal)):
                ax.errorbar(
                    THLListTotal[i],
                    ToTListTotal[i],
                    xerr=THLErrListTotal[i],
                    yerr=ToTErrListTotal[i],
                    color=self.getColor(
                        'Blues',
                        len(ToTListTotal),
                        i),
    ...
    def ToTtoTHL_pixel_DAC(
        ...
        if I_pixeldac:
            dPeripherys = self.split_perihpery_DACs(
                self.peripherys + self.THLs[0], perc=True)
        ...
        for column in range(16):
            for pixel_DAC in pixel_DACs:
                ...
                self.DPX_write_pixel_DAC_command(slot, pixel_DAC * 256, file=False)
            ...
            for i in range(NToT):
                ...
                for pixelx in range(16):
                    for pixely in range(16):
                        conf_bits[lastpixelx, lastpixely] = getattr(
                            ds._ConfBits, 'MaskBit')
                        conf_bits[pixelx, pixely] = getattr(
                            ds._ConfBits, 'TestBit_Analog')
                        self.DPX_write_configuration_command(slot, ''.join(
                            ['%02x' % conf for conf in conf_bits.flatten()]))
                        self.DPXGeneralTestPulse(slot, 1000)
                        lastpixelx, lastpixely = pixelx, pixely
                data = np.asarray(
                    self.DPX_read_ToT_data_dosimode_command(slot), dtype=float)
                data[data > 1000] = np.nan
                dataList.append(data)

            ToT, ToTErr = np.nanmedian(
                dataList, axis=0), np.nanstd(
                dataList, axis=0) / np.sqrt(NToT)
            print(ToT[0])
            ToTListTotal.append(ToT), ToTErrListTotal.append(ToTErr)
            print('Time:', (time.time() - start_time))
        resDict = [{'ToT': np.asarray(ToTListTotal)[:, column, :], 'ToTErr': np.asarray(
            ToTErrListTotal)[:, column, :], 'volt': TPvoltageRange} for column in range(16)]
    ...
    def TPTime(
        ...
        resDict = {column: {'ToT': [], 'ToTErr': [], 'time': timeRange}
                   for column in columnRange}
        conf_bits = np.zeros((16, 16))
        for time in timeRange:
            ...
            for column in columnRange:
                conf_bits.fill(getattr(ds._ConfBits, 'MaskBit'))
                conf_bits[column] = [getattr(
                    ds._ConfBits, 'TestBit_Analog')] * 16 + [getattr(ds._ConfBits, 'MaskBit')] * 0
                self.DPX_write_configuration_command(slot, ''.join(
                    ['%02x' % conf for conf in conf_bits.flatten()]))
                ...
                ToT, ToTErr = np.median(
                    dataList, axis=0), np.std(
                    dataList, axis=0) / np.sqrt(NToT)
    ...
    def TPfindMax(self, slot=1, column=0):
            ...
            while True:
                ...
                if start_flag:
                    ...
                    if scans == 2:
                        ...
                        x, y = list(np.asarray(
                            x)[sort_idx][-5:]), list(np.asarray(y)[sort_idx][-5:])
        ...
        popt, pcov = scipy.optimize.curve_fit(self.linear_fit, np.asarray(voltRange)[
                                              idx], np.asarray(xMaxList)[idx])
        perr = np.sqrt(np.diag(pcov))
        print('Test Pulse energy to Test Pulse duration relation:')
        print('t = (%.2f +/- %.2f) * E_TP + (%.2f +/- %.2f)' %
    ...
    def TPtoToT(
        ...
        resDict = {
            column: {
                'ToT': [],
                'ToTErr': [],
                'volt': TPvoltageRange} for column in columnRange}
            ...
            if DACRange is not None:
                ...
                for DACVal in DACRange:
                    d = self.split_perihpery_DACs(
                        self.peripherys + self.THLs[0], perc=perc)
                    d[DAC] = DACVal
                    code = self.perihery_DACs_dict_to_code(d, perc=perc)
                    self.peripherys = code[:-4]
                    self.DPX_write_periphery_DAC_command(slot, code)
                    ...
                    for column in columnRange:
                        ...
                        for energy in energyRange:
                            ...
                            DACval = self.getTestPulseVoltageDAC(
                                slot, energy, True)
                            self.DPX_write_periphery_DAC_command(slot, DACval)
                            ...
                            for i in range(10):
                                ...
                                self.DPXGeneralTestPulse(slot, 1000)
                                data.append(
                                    self.DPX_read_ToT_data_dosimode_command(slot)[column])
                        ...
                        if False:
                            ...
                            for row in range(16):
                                ...
                                try:
                                    popt, pcov = scipy.optimize.curve_fit(
                                        self.energyToToTFitAtan, x, y, p0=(3, 8, 50, 1))
                                    perr = np.sqrt(np.diag(pcov))
                                except BaseException:
                                    popt = (3, 8, 50, 1)
                                ...
                                plt.plot(
                                    energyRangeFit / float(1000),
                                    self.energyToToTFitAtan(
                                        energyRangeFit / float(1000),
                                        *popt))
                                plt.show()
    ...
    def measureTestPulses(self, slot, column='all'):
        ...
        for column in columnRange:
            ...
            while True:
                self.DPX_data_reset_command(slot)
                # self.DPXGeneralTestPulse(slot, 1000)
                print(self.DPX_read_ToT_data_dosimode_command(slot)[column])

    def measurePulseShape(self, slot, column='all'):
        assert len(self.THL_edges) > 0, 'Need THL calibration first!'
    ...
    def testPulseSigma(self, slot, column=[0, 1, 2], N=100):
        ...
        plt.errorbar(energyRange /
                     1000., [np.mean(item) for item in sigmaListEnergy], xerr=energyRange /
                     1000. *
                     0.18, yerr=np.asarray([np.std(item) for item in sigmaListEnergy]) /
                     np.sqrt(len(columnRange) *
                             16), marker='x', ls='')
    ...
    def testPulseDosi(self, slot=1, column='all'):
        ...
        for energy in energyRange:
            ...
            for column in columnRange:
                ...
                # self.DPX_write_col_sel_command(slot, column)
                ...
                # Read from column
                self.DPX_write_col_sel_command(slot, column)
                out = self.DPX_read_bin_data_dosi_mode_command(slot)
            ...
            for i in range(len(dataMatrix)):
                ...
                plt.imshow(np.asarray([list(entry)
                           for entry in dataMatrix[i]]))
                plt.show()
